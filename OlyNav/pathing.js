function v(startNode, endNode) {
    ...
    // Get length of main floor node array
    const NUMBER_OF_MAIN_FLOOR_NODES=nodes.size;
    ...
    for(var i=0;i<NUMBER_OF_MAIN_FLOOR_NODES;i++) {
        ...
        var coords=new node(nodes.get("a"+i)[0],nodes.get("a"+i)[1],false,false,"a");
    ...
    for(var i=0;i<floorNodes.size;i++) {
        // "nodes" is a map with the name of the node (a15 for example) as key and its coords as values.
        var coords=new node(floorNodes.get("b"+i)[0],floorNodes.get("b"+i)[1],false,false,"b");
        drawNode(coords,i);  //<--- optionally draw the node to screen
    
        if(i==startNode.substring(1)&&startNode.charAt(0)=='b') {coords.isStart=true;var start =i;}
        if(i==endNode.substring(1)&&endNode.charAt(0)=='b') {coords.isGoal=true;var goal = i;}
...
//            datas[i].connections.push(datas[neighbor[j].substring(1)]);
            ...
            if(current.charAt(0)=='a') {
                ...
                console.log((datas[parseInt(neighbor[j].substring(1)+NUMBER_OF_MAIN_FLOOR_NODES)]));
    ...
    while(open.length!=0) {
        ...
        for(var i=0;i<currentNode.connections.length;i++) {
            ...
            if(open.filter(function(e){return e.x==n.x&&e.y==n.y&&e.f_cost(datas[goal])<=n_Cost;}).length>0) {
                //Skip if open list already has a node with this position and lower F cost
      
            }
            else if(closedd.filter(function(ee){return ee.x==n.x&&ee.y==n.y&&ee.f_cost(datas[goal])<n_Cost;}).length>0) {
